...
/// A struct where a `#[repr(u8)]` enum is followed by a `u8` field —
/// if the C header emits the wrong tag width the layout will diverge.
#[ffi]
#[derive(Clone, Copy)]
pub struct ColoredByte {
    pub color: Color,
    pub byte_val: u8,
}
...
#[ffi]
pub fn make_colored_byte(color: Color, byte_val: u8) -> ColoredByte {
    ColoredByte { color, byte_val }
}
...
pub fn inventory() -> RustInventory {
    RustInventory::new()
        .register(function!(make_colored_byte))
