...
pub fn write_pattern_slice(i: &Interop, w: &mut IndentWriter, slice: &CompositeType) -> Result<(), Error> {
    ...
    indented!(w, r"{{ _data = handle.AddrOfPinnedObject(); _len = count; }}")?;
    indented!(w, r"public {}(IntPtr handle, ulong count)", name)?;
    indented!(w, r"{{ _data = handle; _len = count; }}")?;
    ...
    indented!(w, r"{{ Marshal.FreeHGlobal(_data); _data = IntPtr.Zero; }}")?;
    ...
    indented!(w, r"{{ public IntPtr Data; public ulong Len; }}")?;
    w.newline()?;
