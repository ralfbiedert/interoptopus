...
//!
//! Populates immutable `sources` (used by
//! `service_method_overload` to discover overloads) and filtered `ctors` /
//! `methods` lists that exclude non-public functions.
...
impl Pass {
    ...
    pub fn process(
        ...
        // Filter non-public functions from the renderable ctors/methods lists.
        // The source lists remain untouched for overload discovery.
        for service in self.services.values_mut() {
            let old_len = service.ctors.len() + service.methods.len();

            service.ctors.retain(|id| is_public(*id, fns_all));
            service.methods.retain(|id| is_public(*id, fns_all));

            if service.ctors.len() + service.methods.len() != old_len {
                outcome.changed();
            }
        }
...
fn is_public(fn_id: FunctionId, fns_all: &model::common::fns::all::Pass) -> bool {
    fns_all.get(fn_id).map_or(true, |f| f.visibility == Visibility::Public)
}
