...
use crate::lang::plugin::interface::ResultKind;
...
///
/// Returns `(Signature, resolved_rval_id, is_async, result)`:
...
/// - `result`: classification of how a `Result<T, E>` return should be handled at the
///   trampoline boundary (`Try` = unwrapped + `FromCall`, `Passthrough` = kept +
///   `FromCallResult`). See [`ResultKind`] for details. `None` when the rval is not
///   a Result.
///
/// When `unwrap_error_id` is `Some`, Result types whose error matches that id
/// are transparently unwrapped (Try) so the user-facing signature returns `T`.
/// Other Result types are kept (Passthrough).
pub(super) fn resolve_method_info(args: &[Argument], rval: TypeId, types: &TypesAll, unwrap_error_id: Option<TypeId>) -> (Signature, TypeId, bool, Option<ResultKind>) {
    let async_inner = async_callback_inner(args, types);
    let is_async = async_inner.is_some();

    // For async functions, the user-facing return type comes from the AsyncCallback<T>
    // inner type, not the FFI return (which is TaskHandle). When a non-void rval carries
    ...
    // Classify any Result<T, E> on the resolved rval as either Try (peeled) or
    // Passthrough (kept). The two are mutually exclusive by construction.
    let mut result_kind: Option<ResultKind> = None;
    if let Some(ty) = types.get(resolved_rval_id)
        && let TypeKind::TypePattern(TypePattern::Result(ok, err, _)) = &ty.kind
    {
        if let Some(err_id) = unwrap_error_id
            && *err == err_id
        {
            let original = resolved_rval_id;
            resolved_rval_id = resolve_rval_type(*ok, types);
            result_kind = Some(ResultKind::Try(original));
        } else {
            result_kind = Some(ResultKind::Passthrough(resolved_rval_id));
        }
    }
    ...
    (Signature { arguments, rval: effective_rval }, resolved_rval_id, is_async, result_kind)
}
