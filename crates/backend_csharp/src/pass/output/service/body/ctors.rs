        ...
        for (service_id, service) in services.iter() {
            ...
            for ctor_fn_id in &service.ctors {
                ...
                if let Some((overload_id, _transforms)) = async_overload {
                    ...
                    let rendered = templates.render("service/body_ctors_async.cs", &context)?;
...
fn build_args(args: &[crate::lang::functions::Argument], types: &model::types::all::Pass) -> Vec<HashMap<&'static str, String>> {
    args.iter()
        .filter_map(|arg| {
