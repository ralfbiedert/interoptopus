...
impl DotnetLibrary {
    ...
    pub fn process(mut self) -> Result<Multibuf, Error> {
        ...
        loop_model_passes_until_done(|r| {
            ...
            r.run(m.service_type_siblings.process(&mut pass_meta, &mut m.type_kinds, &mut m.type_names, &mut m.type_all, &mut m.type_managed_conversion, &m.service_all, &m.fns_all))?;
            r.run(m.trampoline.process(&mut pass_meta, &m.fns_all, &m.service_all))?;
            r.run(m.plugin_interface.process(&mut pass_meta, &m.trampoline, &m.fns_all, &m.type_all, &m.service_all, &m.service_type_siblings))?;
