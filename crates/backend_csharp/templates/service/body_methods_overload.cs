...
public {{ rval }} {{ method_name }}({% for arg in args %}{{arg.ty}} {{arg.name}}{% if not loop.last %}, {% endif %}{% endfor %})
{
    ...
    try
    {
        {% if not is_void %}return {% endif %}Interop.{{ interop_name }}(_context{% for arg in args %}, {% if arg.is_wrap == "true" %}{{arg.name}}_wrapped{% elif arg.is_ref == "true" %}ref {{arg.name}}{% else %}{{arg.name}}{% endif %}{% endfor %});
    }
    finally
    {
        {% for arg in args %}{% if arg.is_wrap == "true" %}
