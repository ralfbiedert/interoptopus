...
1 + (this.{{field.name}} != null ? Marshal.SizeOf<ulong>() + System.Text.Encoding.UTF8.GetByteCount(this.{{field.name}} ?? "") : 0) /* {{field.kind}} */
{% else %}
1 + (this.{{field.name}} != null ? Marshal.SizeOf<{{field.inner_type}}>() : 0) /* {{field.kind}} of {{field.inner_type}} */
{% endif %}
