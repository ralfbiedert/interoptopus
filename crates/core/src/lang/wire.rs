...
// This is exposed to C# to deallocate memory from C# side (in WireOfT Dispose).
#[unsafe(no_mangle)]
pub unsafe fn deallocate_wire_buffer_storage(data: *mut u8, len: i32, capacity: i32) {
    if capacity <= 0 {
        // If the buffer was borrowed or allocated on the opposite FFI side, cannot deallocate it.
        return;
    }
    let _ = unsafe {
        Vec::from_raw_parts(data, len as usize, capacity as usize);
    };
}
