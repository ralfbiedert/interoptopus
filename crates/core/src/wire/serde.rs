//! Serialization and deserialization traits ([`Ser`] / [`De`]) plus built-in impls for
//! Rust standard library types. User-defined structs annotated with `#[ffi]` get their
//! impls generated by the proc macro.
//!
//! **Note:** Collection length prefixes in these impls use `usize` (platform-width).
//! This is fine for same-machine FFI but means the wire format is not portable across
