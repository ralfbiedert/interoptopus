...
fn closure_call() {
    ...
    let offset = 10;
    let cb = Add::from_closure(move |x, y| x + y + offset);
...
fn closure_call_side_effect() {
    ...
    let cb = Increment::from_closure(move |delta| { c.fetch_add(delta, Ordering::SeqCst); });
...
fn closure_drop_frees_allocation() {
    ...
    let cb = Noop::from_closure(move || { let _ = &captured; });
