...
impl ServiceMultipleCtors {
    pub fn new_with(some_value: u32) -> ffi::Result<Self, FFIError> {
        ffi::Ok(Self { data: vec![some_value; some_value as usize] })
    }

    pub fn new_without() -> ffi::Result<Self, FFIError> {
        ffi::Ok(Self { data: vec![1, 2, 3] })
    }

    pub fn new_with_string(_: ffi::CStrPointer) -> ffi::Result<Self, FFIError> {
        ffi::Ok(Self { data: vec![1, 2, 3] })
    }

    pub fn new_failing(_some_value: u8) -> ffi::Result<Self, FFIError> {
