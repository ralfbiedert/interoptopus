...
impl ServiceVariousSlices {
    ...
    pub fn mut_self_ffi_error(&mut self, _slice: ffi::SliceMut<u8>) -> ffi::Result<(), FFIError> {
        ffi::Ok(())
    }

    pub fn mut_self_no_error(&mut self, mut slice: ffi::SliceMut<u8>) -> ffi::Result<(), FFIError> {
        slice.as_slice_mut();
        ffi::Ok(())
    }
